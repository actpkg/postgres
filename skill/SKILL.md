...
- **TLS (v1):** TLS is supported (pure-Rust rustls + rustls-rustcrypto). However, server certificate **chain** verification is skipped on wasip2 (no system CA store). Handshake signatures are verified, but there is no protection against a MITM presenting any cert. Both `prefer` and `require` behave identically — they attempt TLS with no plaintext fallback. Full chain verification is a planned follow-up.
